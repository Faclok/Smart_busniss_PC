...
// параметры сети
const char* ssid     = "It5G";
const char* password = "itkvantum11";
...
void loop() { 
  ...
  minr = mas[0];
  maxr = mas[1];
  for (byte i = 2; i < LEN; i += 1){ // Ищем мин и макс
      if (mas[i] < minr){ minr = mas[i]; }
      if (mas[i] > maxr){ maxr = mas[i]; }
    }
    
  if (maxr != minr && inOrder == false){
    time1 = millis();
    time2 = millis();
    inOrder = true;
  }
  if (maxr != minr && inOrder == true){    
    time2 = millis();
    Serial.print(" min "); 
    Serial.print(minr);
    Serial.print(" MAX "); 
  ...
  if (maxr == minr && millis() - time2 > 20000 && inOrder == true){
